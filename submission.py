...
# ===== Embedded opponent bot (mimic until turn 100) =====
MIMIC_OLD_UNTIL = 100
_OLD_NS = None
_OLD_SRC = r'''import math
import os
import time
from collections import defaultdict, namedtuple
...
def _get_old_agent():
    ...
    if _OLD_NS is None:
        ns = {}
        exec(compile(_OLD_SRC, "<oldbot>", "exec"), ns, ns)
        _OLD_NS = ns
    return _OLD_NS["agent"]
...
def agent(obs, config=None):
    ...
    # Mimic the embedded opponent bot for the first MIMIC_OLD_UNTIL turns.
    if world.step < MIMIC_OLD_UNTIL:
        try:
            return _get_old_agent()(obs, config)
        except Exception:
