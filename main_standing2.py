...
WIN_RATIO = 1.08    # my ships > best_opp * this -> winning
LOSE_RATIO = 0.92   # my ships < best_opp * this -> losing
...
def _modulate(cfg, standing):
    if standing == "winning":   # cautious: fewer, higher-ROI commits; hold reserve
        return dataclasses.replace(
            cfg,
            roi_threshold=float(cfg.roi_threshold) * 1.8,
            max_waves_per_turn=max(3, int(cfg.max_waves_per_turn) - 3),
            min_ships_to_launch=float(cfg.min_ships_to_launch) + 3.0,
        )
    if standing == "losing":    # aggressive: cheaper commits, more waves
        return dataclasses.replace(
            cfg,
            roi_threshold=max(1.0, float(cfg.roi_threshold) * 0.5),
            max_waves_per_turn=int(cfg.max_waves_per_turn) + 6,
        )
    return cfg
