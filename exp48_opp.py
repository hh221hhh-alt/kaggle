...
TOTAL_STEPS = 500
PHANTOM_FRACTION = 0.5  # opponent model: each enemy planet "phantom-launches" this share
                        # of its ships at its nearest capturable target


def _phantom_enemy_arrivals(obs):
    """Predict the opponent's next move: each enemy planet sends PHANTOM_FRACTION
    of its ships at its nearest planet it doesn't own. Returns tensors for
    record_fleet_arrivals (target_slot, owner, ships, eta) -- to be folded into
    the garrison projection so OUR scoring anticipates the opponent."""
    import torch as _t
    from orbit_lite.geometry import fleet_speed as _fs
    P = int(obs.P)
    x = obs.x; y = obs.y; ships = obs.ships
    owner = obs.owner_abs; alive = obs.alive
    pid = int(obs.player_id)
    dev = obs.device; dt = ships.dtype
    if P == 0:
        z = _t.zeros(0, device=dev)
        return z.long(), z.long(), z, z
    dx = x.view(P, 1) - x.view(1, P)
    dy = y.view(P, 1) - y.view(1, P)
    dist = _t.sqrt((dx * dx + dy * dy).clamp(min=0.0))
    eye = _t.eye(P, device=dev, dtype=_t.bool)
    same_owner = owner.view(P, 1) == owner.view(1, P)
    valid_tgt = alive.view(1, P) & (~same_owner) & (~eye)
    big = _t.full_like(dist, float("inf"))
    dmask = _t.where(valid_tgt, dist, big)
    tgt = dmask.argmin(dim=1)                                   # [P]
    has_tgt = _t.isfinite(dmask.gather(1, tgt.view(P, 1)).squeeze(1))
    enemy = alive & (owner >= 0) & (owner != pid)
    send = (float(PHANTOM_FRACTION) * ships).floor()
    src = enemy & has_tgt & (send >= 1.0)
    d_to_tgt = dist.gather(1, tgt.view(P, 1)).squeeze(1)
    speed = _fs(send.clamp(min=1.0)).clamp(min=1e-6)
    eta = _t.ceil(d_to_tgt / speed).clamp(min=1.0)
    idx = _t.where(src)[0]
    return tgt[idx], owner[idx].long(), send[idx].to(dt), eta[idx].to(dt)
...
def run_turn(obs_tensors: dict, *, config: ProducerLiteConfig, player_count: int, memory) -> dict:
    ...
    # opponent-aware projection: a FRESH movement (untracked, discarded each turn)
    # with phantom enemy launches folded into its garrison forecast.
    opp_cfg = dataclasses.replace(_movement_config(config, player_count=int(player_count)),
                                  track_fleets=False)
    opp_mv = PlanetMovement.from_obs_tensors(obs_tensors, config=opp_cfg)
    _tgt, _own, _shp, _eta = _phantom_enemy_arrivals(obs)
    if int(_tgt.numel()) > 0:
        opp_mv.record_fleet_arrivals(target_slots=_tgt, owner_ids=_own, ships=_shp, eta=_eta)
    status = opp_mv.garrison_status(max_horizon=H)
    alive_by_step = opp_mv.alive_by_step[: H + 1]
