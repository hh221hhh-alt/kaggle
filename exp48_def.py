...
TOTAL_STEPS = 500
DEF_K = 0.5  # opponent-aware: reserve this x reachable-enemy-mass on each source
...
def plan_lite_waves(
    *,
    ...
    # opponent-aware reserve: keep more on planets the enemy can REACH (1-ply
    # threat), not just where in-flight fleets already head -> less over-extension.
    _ep = cheap_enemy_pressure(obs, cache, horizon=float(K_eta), player_id=pid)
    drain = (drain - DEF_K * _ep[source_idx.clamp(0, P - 1)]).clamp(min=0.0)
