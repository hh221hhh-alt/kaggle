...
class _Brain(nn.Module):
    def __init__(self):
        super().__init__()
        self.shared = nn.Sequential(
            nn.Linear(_NN_N*_NN_F, 256), nn.LayerNorm(256), nn.ReLU(),
            nn.Linear(256, 256), nn.ReLU(), nn.Linear(256, 128), nn.ReLU())
        self.policy = nn.Linear(128, _NN_N*_NN_N)
...
_NN_MODEL = None
try:
    _wp = os.path.join(_HERE, "orbit_brain.pth")
    _m = _Brain()
...
def _nn_features(obs, obs_tensors, pid):
    P = int(obs.P)
    x = torch.zeros(_NN_N, _NN_F, dtype=torch.float32)
    own = torch.zeros(P)
    own[obs.owned] = 1.0
    enemy = obs.alive & (obs.owner_abs >= 0) & (obs.owner_abs != int(pid))
    own[enemy] = -1.0
    px = obs.x.float(); py = obs.y.float(); ships = obs.ships.float()
    pl = obs_tensors["planets"]
    prod = pl[:, 6].float() if pl.shape[1] > 6 else torch.zeros(P)
    rad = pl[:, 4].float() if pl.shape[1] > 4 else torch.zeros(P)
    dist = torch.hypot(px - _NN_CENTER, py - _NN_CENTER)
...
def _nn_bias(obs, obs_tensors, pid, alpha):
    if _NN_MODEL is None or alpha == 0.0:
        return None
    P = int(obs.P)
    feats = _nn_features(obs, obs_tensors, pid).unsqueeze(0)
    with torch.no_grad():
        logit, _ = _NN_MODEL(feats)
    lp = _Fnn.log_softmax(logit.view(_NN_N, _NN_N), dim=-1)  # per-source over targets
    P2 = min(P, _NN_N)
    out = torch.full((P, P), 0.0, dtype=torch.float32)
    out[:P2, :P2] = lp[:P2, :P2] * float(alpha)
    return out
...
def _tier_candidates(
    *,
    ...
    if nn_bias is not None:
        _ss = cand_src.view(-1).clamp(0, nn_bias.shape[0]-1)
        _ts = cand_tgt_slot.clamp(0, nn_bias.shape[1]-1)
        _b = nn_bias[_ss, _ts]
        _b = torch.where(cand_is_def, torch.zeros_like(_b), _b)  # offensive targets only
        score = score + _b
...
def plan_lite_waves(
    *,
    ...
    nn_bias = _nn_bias(obs, obs_tensors, pid, float(getattr(config, 'nn_alpha', 0.0)))
